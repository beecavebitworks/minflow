...
static int isProbablePrime(uint32_t number);
static uint32_t calculateIdealNumOfBuckets(HashTable *hashTable);
...
HashTable *HashTableCreate(uint32_t numOfBuckets) {
    HashTable *hashTable;
    int i;
...
void *HashTableGet(const HashTable *hashTable, const void *key) {
    uint32_t hashValue = hashTable->hashFunction(key) % hashTable->numOfBuckets;
 ...
 *  RETURNS:
 *      uint32_t         - the number of key/value pairs that are present in
 *                     the specified HashTable
\*--------------------------------------------------------------------------*/

uint32_t HashTableSize(const HashTable *hashTable) {
 ...
 *  RETURNS:
 *      uint32_t         - the number of buckets that are in the specified
 *                     HashTable
\*--------------------------------------------------------------------------*/

uint32_t HashTableGetNumBuckets(const HashTable *hashTable) {
...
void HashTableRehash(HashTable *hashTable, uint32_t numOfBuckets) {
    ...
    for (i=0; i<hashTable->numOfBuckets; i++) {
        ...
        while (pair != NULL) {
            KeyValuePair *nextPair = pair->next;
            uint32_t hashValue = hashTable->hashFunction(pair->key) % numOfBuckets;
 ...
 *  RETURNS:
 *      uint32_t - the unmodulated hash value of the key
\*--------------------------------------------------------------------------*/
...
static uint32_t pointerHashFunction(const void *pointer) {
    return ((uint32_t) (uint64_t)pointer) >> 4;
}

static int isProbablePrime(uint32_t oddNumber) {
    uint32_t i;
...
static uint32_t calculateIdealNumOfBuckets(HashTable *hashTable) {
    uint32_t idealNumOfBuckets = hashTable->numOfElements / hashTable->idealRatio;
